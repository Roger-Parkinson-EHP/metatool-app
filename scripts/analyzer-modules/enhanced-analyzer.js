...
const codeMetrics = require('./analyzer-modules/code-metrics');
...
<body>
  <div class="container">
    ...
    ${results.codeQualityAnalysis ? `
    <div class="card">
      <h2>Code Quality Analysis</h2>
      <div class="stats">
        <div class="stat-card">
          <div>Total Code Files</div>
          <div class="number">${results.codeQualityAnalysis.metrics.fileMetrics.length}</div>
        </div>
        <div class="stat-card">
          <div>Average Complexity</div>
          <div class="number">${results.codeQualityAnalysis.metrics.summary.averageComplexity.toFixed(1)}</div>
        </div>
        <div class="stat-card">
          <div>Refactoring Candidates</div>
          <div class="number">${Object.values(results.codeQualityAnalysis.refactoringCandidates).flat().length}</div>
        </div>
      </div>
      ...
      <table>
        ...
        ${results.codeQualityAnalysis.refactoringCandidates.highComplexity
          .slice(0, 10)
          .map(file => `<tr>
            <td>${file.filePath.replace(config.rootDir, '')}</td>
            <td class="warning">${file.complexity}</td>
...
async function runAnalysis(options = {}) {
  ...
  // Run code quality analysis if requested
  if (config.analysisModules.includes('all') || config.analysisModules.includes('quality')) {
    console.log('\n=== Running Code Quality Analysis ===');
    try {
      results.codeQualityAnalysis = await codeMetrics.analyzeCodeQuality(results.repoStructure.repoStructure, config);
    } catch (error) {
      console.warn('Code quality analysis module not available:', error.message);
    }
  }
